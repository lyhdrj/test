...
|--------------------------------------------------*/
function getData() {
	menuDao.getUserMenus($("userId").value,showData);
}
function showData(data) {
	if (data != null) {
		d = new dTree('d','/projectManageSystem/images/tree/');
		for(var i=0;i<data.length;i++){
			//if(data[i].parentMenu==null){
				//d.add(data[i].menuNo, -1, data[i].menuName, data[i].linkURL, null, data[i].frameTarget);
			//}else{
...
function dTree(objName,iconPath) {
	...
	if (!iconPath)
		iconPath = "/projectManageSystem/images/tree/";// set the default value, be compatible with dtree
	this.icon = {
	...
	if (node.url) {
		str += '<a id="s'
				...
				+ ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node'))
						: 'node') + '" href="javascript:void(0);" onclick=show("' + node.url + '","' + node.target + '")';
		if (node.title)
